...
func SetBenchmark(ctx context.Context, cache *fluxcache.FluxCache, totalUsers, concurrency int) {
	fmt.Println("Starting SET benchmark...")

	start := time.Now()

	var wg sync.WaitGroup
	setCh := make(chan int, totalUsers)
	...
	for i := 0; i < concurrency; i++ {
		...
		go func() {
			...
			for id := range setCh {
				...
				err := cache.SetProto(fmt.Sprintf("user:%d", id), user)
				if err != nil {
